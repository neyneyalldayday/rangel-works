    ...
    const handleErrors = errors => {
        if (!Array.isArray(errors) && !errors.length > 0) {
            return(
                ...
                error 
                header="Sorry"
                content="Please check your login daddy and try again"
                />
            )
        }
        return errors.map(e => (
            ...
            >
                ...
                <Segment>
                    ...
                    <Form.Field>
                     ...
                     {errors.password && <p style={{color: 'red'}}>{errors.passsword}</p>}
...
const validate = values => {
    const errors = {}
    if(!values.email) {
        errors.email = 'Email address is required daddy'
    } else if (!/\S+@\S+\.\S+/.test(values.email)) {
        errors.email = 'Email address is invalid'
    }
    if (!values.password) {
        errors.password = 'Password is Required daddy'
    }
}